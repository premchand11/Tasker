...
import (
	...
	"github.com/premchand11/tasker/internal/config"
	"github.com/rs/zerolog"
	"github.com/rs/zerolog/pkgerrors"
)

// LoggerService manages newrelic integration and logger creation
...
func NewLoggerService(cfg *config.ObservabilityConfig) *LoggerService {
	...
	if cfg.NewRelic.LicenseKey == "" {
		fmt.Println("new relic license_key is not provided ,skipping intilization")
		return service
	}
...
func NewLoggerWithService(cfg *config.ObservabilityConfig, loggerService *LoggerService) zerolog.Logger {
	...
	if cfg.IsProduction() && cfg.Logging.Format == "json" {
		...
		writer = baseWriter //we chnaged thsi need keep an eye if anythingg got broke------------------------
	} else {
